...
module.exports = class DotaBot {
    ...
    constructor (logonDetails, debug, debugMore) {
        ...
        this.Dota2              = new dota2.Dota2Client(this.steamClient, this._debug, this._debugMore)
        ...
        let onConnected = function onConnected() {
                if (self._debug) {
                    logger.debug('Connected, logging on..')
                }
            ...
            onSteamLogOn    = function onSteamLogOn(logonResp){
                ...
                // Set status to online
                    ...
                    // Set nickname
                    if (self._logonDetails.persona_name) {
                        self.steamFriends.setPersonaName(self._logonDetails.persona_name)
                    }

                    if (self._debug) {
                        logger.debug('Logged on with id = ' + self.Dota2 .ToAccountID(self.Dota2._client.steamID))
                    }
                    self.Dota2.launch()
            ...
            onSteamServers  = function onSteamServers(servers) {
                if (self._debug) {
                    logger.debug('Received servers.')
                }
                fs.writeFileSync('servers', JSON.stringify(servers))
            },
            onSteamLogOff   = function onSteamLogOff(eresult) {
                if (self._debug) {
                    logger.debug('Logged off from Steam. Trying reconnect with result: ' + eresult)
                }
